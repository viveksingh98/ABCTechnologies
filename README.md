...
1.	Create a Dockerfile in the root directory of your project with the following content:
    ...
    EXPOSE 8080
    CMD ["catalina.sh", "run"]
2.	Commit the Dockerfile to the Git repository.
3.	Install Docker on the Jenkins machine and ensure that the Jenkins user has sufficient permissions to execute Docker commands.
4.	Install the Docker plugin on Jenkins by navigating to "Manage Jenkins" -> "Manage Plugins" -> "Available" -> "Docker plugin" and install.
5.	Configure Docker in Jenkins by navigating to "Manage Jenkins" -> "Configure System" -> "Docker" section. Add Docker installation and give a name.
6.	Create a new freestyle job in Jenkins and configure it as follows:
...
7.	Save and run the job to build a Docker image.
8.	Once the Docker image is built, I can use it to create a container using the following command:
      sudo docker run -p 8081:8081 -d dockerfile
9.	I can access the running container by navigating to http://<docker-host>:8081/. (I have set port 8081 because on port 8080 Jenkins is running)
10.	I have now successfully integrated Docker with Jenkins and created a CI/CD pipeline to build and deploy a Docker container.
...
2. I used the "Package" job as the upstream to configure this job.
3. The "/opt/ansible" directory was used to move the "\*.war" package.
4. A Dockerfile was created to build an image with the "\*.war" package and run it on a Tomcat container.
5. An Ansible playbook was added to create and push the Docker image to Docker Hub.
6. In the Jenkins job, I executed the Ansible playbook only after the "Package" job had been completed successfully.
7. Using Ansible, I created a Docker image with the "\*.war" package and pushed it to Docker Hub.
8. I also created Kubernetes service and deployment manifest files in the "/opt/k8s" directory.
9. An Ansible playbook was created to deploy Kubernetes resources.
10. A Jenkins job was created to automate the execution of the playbooks to achieve continuous deployment using the "ABC\_Technologies\_Ansible\_job\_to\_push\_image\_onto\_dockerhub" job as the upstream.
11. I executed the Ansible playbook to deploy on the K8s cluster only if the upstream job was stable.
12. The output of the execution was observed and verified.
...
2. Rename the extracted tar file to node\_exporter-files.
3. Run the node\_exporter script.
4. Add the node\_exporter target to the prometheus.yaml file.

5. Create Grafana dashboard.
6. Add the source as Prometheus.
